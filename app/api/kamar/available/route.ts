...
type KamarRow = { id: number; no_kamar: string | null; jenis_bed: string; harga: number; max_tamu: number };
...
export async function GET(req: Request) {
  ...
  if (!checkin || !checkout) {
    return NextResponse.json({ message: 'checkin dan checkout wajib diisi' }, { status: 400 });
  }

  if (new Date(checkin) >= new Date(checkout)) {
    return NextResponse.json({ message: 'checkout harus setelah checkin' }, { status: 400 });
  }
  ...
  try {
    ...
    return NextResponse.json({ message: 'Gagal mengambil data kamar tersedia' }, { status: 500 });
  }
}
